...
The system includes a composable data-fetch interface that normalizes outputs across sources (US + India), prefers cache with sensible TTLs, and falls back to network with polite rate limiting.

### Cache-First Architecture

- **Lookup Order**: In-memory hot cache → SQLite http_cache → Network
- **TTL Policies**:
  - Price data: 1 day
  - Fundamentals: 90 days
  - Instrument metadata: 30 days
  - SEC filings: 365 days
- **Freshness Check**: `fetched_at + ttl_days > now`

### Unified Fetch Interface

```python
from multibagger.data import DataFetcher

fetcher = DataFetcher()

# Get price data
prices = fetcher.get_prices(['AAPL', 'MSFT'], start_date, end_date)

# Get fundamentals
fundamentals = fetcher.get_fundamentals(['AAPL'], period='annual')

# Get instrument metadata
instruments = fetcher.get_instruments(['US'])

# Get SEC filings
filings = fetcher.get_filings('AAPL', form_types=['10-K', '10-Q'])
```

### Cache-Only Mode

For offline runs or reproducible analysis:

```bash
# Warm cache for current month
multibagger data warm 2025-11

# Fetch in cache-only mode
multibagger data fetch-prices --tickers AAPL,MSFT --start 2025-01-01 --end 2025-11-01 --cache-only

# Check cache stats
multibagger data stats
```

### Data Sources

- **Yahoo Finance**: Prices and instrument metadata (free, no rate limits)
- **Alpha Vantage**: Backup for prices and fundamentals (5 calls/min free tier)
- **SEC EDGAR**: Filing metadata and document links (10 calls/min)
- **Screener.in**: Indian market fundamentals (60 calls/min)
